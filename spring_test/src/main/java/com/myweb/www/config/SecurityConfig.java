...
public class SecurityConfig extends WebSecurityConfigurerAdapter{
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		// 승인 요청
		//antMatchers : 접근을 허용하는 값 / 결제요청이나 상품등록같은거 여기에 추가
		//permitAll() : 누구나 접근 가능한 경로
		//authenticated() : 인증된 사용자만 가능(로그인한 사람)
		http.authorizeRequests()
		.antMatchers("/member/list").hasRole("ADMIN")
		.antMatchers("/", "/board/list", "/board/detail", "/comment/**", "/upload/**", "/resources/**", "/member/register", "/member/login","/member/**").permitAll()
		.anyRequest().authenticated(); //board/register하려면 login해야해서 자동으로 login.jsp로 넘어가야함
